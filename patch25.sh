...
require_file "runtime/bin/sign_detached"
require_file "runtime/bin/verify_detached"
require_file "runtime/bin/verify_release_signatures"
...
note "Inspecting runtime/bin/attach_release_signatures header + suspicious lines…"
head -n 5 runtime/bin/attach_release_signatures | sed 's/^/   /' >&2 || true
grep -nE 'signing_alg' runtime/bin/attach_release_signatures | sed 's/^/   /' >&2 || true

# Detect corruption: line containing d["signing_alg" without ] =
if grep -qE 'd\["signing_alg"[^]]' runtime/bin/attach_release_signatures; then
  note "attach_release_signatures proven CORRUPT -> rewriting cleanly"
  backup runtime/bin/attach_release_signatures

  cat > runtime/bin/attach_release_signatures <<'PY'
#!/usr/bin/env python3
...
def main() -> int:
    ...
    bundle_abs = bundle_path if os.path.isabs(bundle_path) else os.path.normpath(os.path.join(mdir, bundle_path))
    ...
    # Re-validate after attach (positional CLI)
    rc, out, err = run(["./runtime/bin/validate_manifest", mpath])
    if rc != 0:
        return die(f"manifest failed validation after attach:\n{err.strip()}")
...
PY
  ...
  note "attach_release_signatures not detected as corrupt by pattern (still may be wrong, but not corrupted)"
fi

note "Re-inspect attach_release_signatures signing_alg lines (should be clean now)…"
grep -nE 'signing_alg' runtime/bin/attach_release_signatures | sed 's/^/   /' >&2 || true

note "Run golden proof chain (authoritative)…"
