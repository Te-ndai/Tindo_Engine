...
F="patch102_hygiene.sh"
[[ -f "$F" ]] || die "missing: $F (your root-run hygiene script)"

cp -a "$F" "$F.bak.$(date -u +%Y%m%dT%H%M%SZ)"

# Overwrite with a robust version (same interface)
cat > "$F" <<'SH'
#!/usr/bin/env bash
...
chmod +x "$F"
note "Rewrote $F to robust/resumable apply behavior."
note "Now run: ./$F --apply"
