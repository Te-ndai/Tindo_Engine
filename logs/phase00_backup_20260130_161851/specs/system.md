...
## Purpose
This document declares what exists in the system (inventory), not how it is implemented. It is a Phase 0 contract. :contentReference[oaicite:1]{index=1}

## Root and Pathing
- The human chooses a project root directory (“ROOT/”).
- All paths referenced in this repository are **relative to ROOT/**.
- No artifact may depend on anything outside ROOT/. :contentReference[oaicite:2]{index=2}
...
- runtime/      (final runnable system only; no scripts/tests/specs) :contentReference[oaicite:3]{index=3}

Nothing else is part of the system.

## Runtime: What Must Exist
runtime/ is the only executable deliverable. It must stand alone. :contentReference[oaicite:4]{index=4}
...
- Host adapters must invoke exactly this entrypoint. :contentReference[oaicite:5]{index=5}

## System Composition (Non-negotiable)
System = Core ⊗ HostAdapters ⊗ PathModel ⊗ Lattice :contentReference[oaicite:6]{index=6}
...
- logs/runtime.manifest.json :contentReference[oaicite:7]{index=7}

## What This System Is (and is not)
- This system is a deterministic construction factory that produces a runtime.
- It is not “a collection of plans.” Plans must compile into scripts + tests + runtime artifacts. :contentReference[oaicite:8]{index=8}
