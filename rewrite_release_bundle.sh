...
# Find repo root by walking upward until runtime/bin/release_bundle exists
find_root() {
  local d="$PWD"
  while true; do
...
ROOT="$(find_root)" || die "could not locate repo root containing runtime/bin/release_bundle from: $PWD"
cd "$ROOT"

F="runtime/bin/release_bundle"
[ -f "$F" ] || die "missing $F (unexpected after root detection)"
...
# Manifest pre-hash (derive expectations from append-only chain, not projections)
python3 - <<PY
import json, os
...
def last_event_time_from_chain(p: str) -> str:
...
m = {
  ...
  # Explicit restore/replay expectations (Phase 84 contract)
